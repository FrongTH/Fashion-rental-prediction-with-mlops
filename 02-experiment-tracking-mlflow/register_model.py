...
mlflow.set_experiment("fashion-rental-prediction-best-models")
...
SPACE = {
        'n_neighbors': scope.int(hp.quniform('n_neighbors', 1, 15, 1)),
...
def run(config_path):
    ...
    client = MlflowClient()
    experiment = client.get_experiment_by_name("fashion-rental-prediction-hyperopt") #hpo experiment
    runs = client.search_runs(
        experiment_ids=experiment.experiment_id,
        run_view_type=ViewType.ACTIVE_ONLY,
        max_results=config['TOP_N'],
        order_by=["metrics.root_mean_squared_error"]
    )

    for run in runs:
        train_and_log_model(data_path=glob(os.path.join('../data/split/*'))[-1], params=run.data.params)

    # select the model with the lowest test RMSE
    experiment = client.get_experiment_by_name("fashion-rental-prediction-best-models")
    ...
    run_id = best_run.info.run_id ## error here
